...
## 1.Defining Functions for Pulling, Processing, and Creating Techincial Indicators
...
def calculate_metrics(data):
    # Ensure we're using the last row for calculations
...
def add_technical_indicators(data):
    # Ensure data is a DataFrame and we're using the 'Close' column
...
## 2.Creating the Dashboard App layout
...
for symbol in stock_symbols:
    ...
    if not real_time_data.empty:
        ...
        last_price = real_time_data['Close'].iloc[-1].item()  # Use .item() to extract scalar value
        open_price = real_time_data['Open'].iloc[0].item()  # Use .item() to extract scalar value
